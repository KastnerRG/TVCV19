...
namespace TvCv19.Frontend.Hubs
{
    public class ChatHub : Hub
    {
        ...
        public async Task SendMessageAsync(string patientId, string physicianId, string message, bool isCareInstruction, bool isAudio)
        {
            var date = DateTime.Now;
            var physician = await _physicianRepository.GetPhysicianAsync(physicianId);
            var id = await _messageRepository.AddMessage(new Message(patientId, message, physician.Name, date, isCareInstruction, isAudio));
            await Clients.Group(patientId).SendAsync("ReceiveMessage", message, physician.Name, date, id, isCareInstruction, isAudio);
        }
    }
}
